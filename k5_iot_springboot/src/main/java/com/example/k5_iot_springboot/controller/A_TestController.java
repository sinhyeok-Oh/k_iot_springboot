...
// @Controller
// : 웹 요청을 처리하는 클래스 임을 명시 (반환되는 데이터 타입이 유언 - JSP, Thymeleaf 등)
// @ResponseBody
...
public class A_TestController {
    ...
    // @메서드Mapping("추가URI지정")
    // : 메서드(POST/GET/PUT/DELETE) + localhost:8080/RequestMapping결로/추가URI
    @PostMapping
    ...
    // 요청 구조: HTTP 메서드 + URI 결로 (URI 자원에 어떠한 HTTP 동작을 실행할 것인지 명시하여 요청)
