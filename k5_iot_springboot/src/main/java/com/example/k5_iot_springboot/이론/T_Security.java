...
/*
    ...
    : Spring Framework 기반 에플리케이션에서 보안(인증, 인가, 권한)을 담당하는 보안 프레임워크
    ...
    3. 유현한 확장성: 소셜 로그인, OAuth2, JWT, 2FA 등으로 확장 가능.
    4. 필수 방어막: 인증/인가 없는 서비스는 보안 취약

    === Security 선행 개념 ===
    1. HTTP는 무상태(Stateless)
        : 인터넷의 HTTP 통신은 기억력이 없음!
        - 서버는 이전 요청에 대한 사용자 정보를 저장 기억하지 않음 (매번 로그인 정보를 다시 제출해야 함)
    ...
    3. 암호화 / 해싱
        - 비밀번호 암호화 (문자열 그대로 저장하면 보안성 저하)
            ...
            >> 스프링 시큐리터는 주로 BCrypt 사용 (매번 다른 해시 결과로 만들어짐 - 안전성 향상)

    4. Principal (주요한)
        - "인증된" 사용자 정보 (아이디, 이메일 등)
    5. Authority(권한), ROLE(역할)
        - 역할에 따라 권한이 달라짐
        >> 해당 요청 (해당 URL)은 권한 ROLE_ADMIN 만 들어갈수 있음의 규칙을 정의하는 것
    6. SecurityContext
        : 현재 요청의 인증/권한 저장
        - 현재의 사용자 기록
    7. Filter Chain
    ...
    2) SecurityFilterChain - 검문소 라인
        ...
        - JWT 토큰이 있는지(JwtAuthenticationFilter, 커스텀 Filter 에 전달)
        - 보호된 URL인지?
    ...
    5) PasswordEncoder - 비밀번호 검증 담당
        ...
        - 사용자가 입력한 비밀번호와 DB 해기값을 Encoder가 바교
            >> 일치하면 인증 성공, 틀리면 실패
    ...
    7) Authorities (권한) - 어디까지 접근할 수 있는지 확인
        : UserDetails 안에 유저가 가진 구너한(ROLE) 정보 저장
        - 필터체인은 요청된 URL과 권한을비교
            >> 맞으면 통과, 아니면 막힘
