...
*   , HTTP 메서드를 이요해 해당 자원에 대한 행위를 정의하는 아카텍처 스타일
*
*   - 자원의 표현: JSON, XML 등
...
*   (0) /users, /products, /carts, /meetings 등
...
*   (0) /user-profiles, /school-numbers
*
* 4) 마지막 / 금지 - URI 끝에 /를 남기지 않음
*   (0) /users/1, /products/search/name
*
* 5) 계층 구조 표현 - 관계는 / 로 표현
*   (0) /users/{userId}/orders/{orderId} - 특징 사용자(userId)의 주문들 중 특정 주문(orderId)
*
* 6) 복수형 명사 사용 - 특정 데이터는 복수형 명사 뒤 계층 구조로 표시
*   (0) /users, /posts, /books, /menus
*
...
*   - 이메일 중복 체크: GET /api/v1/auth/check-email=XXX...)
*
* 케이스2) 쇼핑몰
*   - 상품 목록: GET /api/v1/products
*   - 상품 리뷰 조회: GET /api/v1/products/{productId}/reviews
*   - 장바구니: GET /api/v1/users/me/cart (사용자 1인 당 장바구니는 1개)
*   - 주문 생성: POST /apo/v1/orders
*
* 케이스3) 게시판
*   - 게시판 목록: GET /api/v1/post?category=notice&search=java&page=1&size=10
