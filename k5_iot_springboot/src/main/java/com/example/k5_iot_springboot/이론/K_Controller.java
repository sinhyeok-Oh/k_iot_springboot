...
/*
    1. @PathVariable
    : 경로 변수
    : URI 자원 경로 자체에 포함된 변수를 매핑하여 받은 어노테이션
    EX) GET "/books/{isbn}"
        : 책들 중 해당 isbn의 책을 GET 가져오기
        - 특정 리소스에 접근, 수정, 삭제에 사용
    >> GET, PUT, DELETE 사용 (POST 사용하지 않는다)
    ...
    cf) 옵션
    : 변수명과 파라이미터명이 다를 경우 @PathVariable("이름") @PathVariable("isbn") Long bookId

    2. @RequestBody
    : 클라이언트의 HTTP 요청 본문(Body)에 담긴 JSON, XML 데이터를
        , 자바 객체로 변환하여 메서드의 파라미터로 매핑할 때 사용
    - JSON, XML 형태의 데이터를 DTO 객페로 자동 변한 (RequestDto)
    ...
    3. @RequestParam
    :클라이언트 보낸 URL 쿼리 스트링 또는 폼 데이터를 메서드 파라미터로 바인딩 할 때 사용
    - URL에 노출되기 때문에 민감하지 않은 데이터에 접합
    - 주로 GET 요청에 주로 사용
